...
@section('title')
    НЧПК | Библиотека | Редактирование отзыва
@endsection
    ...
    <section class="main_block_lk element_pad">
        <div class="container">
            <div class="lk_inner_block">
                ...
                @endif
                <form action="{{route('user.feedbacks.update', $feedback)}}" method="post" class="book_info_section_reviews_form">
                    @method('patch')
                    ...
                    <div class="comment_form_rating_input_control">
                        <textarea id="comment" name="message" placeholder="Текст отзыва" cols="100" rows="10">{{$feedback->message}}</textarea>
                    </div>
                    @error('message')
                    <div class="text-danger">
                        {{$message}}
